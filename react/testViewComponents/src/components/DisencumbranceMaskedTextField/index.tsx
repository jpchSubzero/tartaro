...
import moduleStyles from "./DisencumbranceMaskedTextField.module.scss";
...
export default function DisencumbranceMaskedTextField(props) {
  ...
  const getEndInputAdorment = () => {
    ...
    if (!props.endAdormentIcon && !props.error) {
      return null;
    }
    let endAdormentIconToRender = props.endAdormentIcon ? (
      props.endAdormentIcon
    ) : (
      <ErrorOutlineIcon />
    );
    let endAdormentIconClassName = props.error
      ? moduleStyles["EvaTextField-EndAdorment--error"]
      : moduleStyles["EvaTextField-EndAdorment"];
  ...
  const startInputAdorment = props.startAdormentIcon ? (
  ...
  return (
    <>
      <EvaTypography
        ...
        className={moduleStyles["EvaTextField-fieldlabel"]}
      >
        {props.fieldlabel}
      </EvaTypography>
