import { IProductResponse, IProductResponseResultValue } from '../interfaces/products/product.response.interface';
...
const products = getFromEnvFile(productCodeKey).split(',');
...
async function getProductByConfiguredFiltersCodeAsync() {
    ...
    if (responseProducts.result) {
        let filteredProducts:IProductResponseResultValue[] = [];
        responseProducts.result.$values.forEach(product => {
            if (products.includes(product.code)) {
                filteredProducts.push(product);
            }
        });
        responseProducts.result.$values = filteredProducts;
